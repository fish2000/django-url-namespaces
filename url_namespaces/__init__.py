...
def modulize(ns):
    ...
    print "ns.namespace = %s" % ns.namespace
    print "ns_modname = %s" % ns_modname
    
    sys.modules.update({ ns_modname: ns_module, })
...
def include(arg, namespace=None, app_name=None):
    ...
    if isinstance(arg, basestring):
        try:
            ...
            return (mod, include_namespace, include_app_name)
...
class ArgumentSink(object):
    ...
    def evaluate(self):
        return tuple(self._args) + tuple(self._kwargs.items())
        ...
        if isinstance(self.action, basestring):
            if isinstance(self._options.get('view_prefix'), basestring):
                if len(self._options.get('view_prefix')) > 0:
                    new_action = ".".join(
                        [self._options.get('view_prefix'), self.action])
...
class Namespacer(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for name, value in filter(
            lambda nv: isinstance(nv[1], ArgumentSink),
            attrs.items()):
